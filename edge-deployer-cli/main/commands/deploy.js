...
module.exports = function deploy(functionName, infrastructure, {inEvery, inAreas, exceptIn, yaml}) {
    try {
        ...
        for(const location of listOfLocations) {
            const envVariablesString = "--env=LOCATION_ID=" + location.location_id + " --env=EDGE_DEPLOYMENT_IN_EVERY=" + inEvery + " --env=EDGE_INFRASTRUCTURE='" + infrastructureString + "' --env=REDIS_HOST=" + location.redis_host + " --env=REDIS_PORT=" + location.redis_port + " --env=REDIS_PASSWORD=" + location.redis_password;
